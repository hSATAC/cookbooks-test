...
node.default["nginx"]["passenger"]["enterprise"] = false
...
if node["nginx"]["passenger"]["enterprise"]
  ...
  if ruby_version[0..2] == 'ree'
    compatible_version = "1.8"
  else
    compatible_version = ruby_version[0].to_f == 1 ? "1.9.1" : "2.0.0"
  end
  node.default["nginx"]["passenger"]["root"] = "/usr/local/rbenv/versions/#{ruby_version}/lib/ruby/gems/#{compatible_version}/gems/passenger-enterprise-server-#{node['nginx']['passenger']['version']}"
  node.default["nginx"]["passenger"]["ruby"] = "/usr/local/rbenv/versions/#{ruby_version}/bin/ruby"

  include_recipe "passenger-enterprise"
  include_recipe "passenger-enterprise::nginx"
else
